...
class TimestampConverter(commands.Cog):
    ...
    @app_commands.command(name="my_timezone", description="Check your current timezone setting")
    async def my_timezone(self, interaction: discord.Interaction):
        """Show user's current timezone setting."""
        try:
            user_tz = self.user_timezones.get(interaction.user.id)
            
            if not user_tz:
                embed = discord.Embed(
                    title="⚠️ No Timezone Set",
                    description="You haven't set a timezone yet.\n\nUse `/set_timezone` to set your timezone for accurate timestamp conversion!\n\nExample: `/set_timezone EST` or `/set_timezone America/New_York`",
                    color=discord.Color.orange()
                )
            else:
                # Get current time in user's timezone
                user_zone = ZoneInfo(user_tz)
                current_time = datetime.now(user_zone).strftime('%I:%M %p on %B %d, %Y')
                
                embed = discord.Embed(
                    title="🌍 Your Timezone",
                    description=f"**Timezone:** {user_tz}\n**Current time:** {current_time}\n\nWhen you mention times in chat, they'll be converted based on this timezone.",
            ...
            logger.error(f"Error in my_timezone: {e}")
            await interaction.response.send_message("❌ An error occurred while checking your timezone.", ephemeral=True)
