...
# --- Added: sensible defaults and lightweight UI/helper stubs for imports -----------------
# Provide a default STEAM_API_KEY (can be overridden via env) so cogs importing it won't fail.
STEAM_API_KEY = os.getenv("STEAM_API_KEY", "")

# Path to DB used by cogs (aio-sqlite accepts Path or str). Override via env if needed.
DB_PATH = os.getenv("STEAM_DB_PATH", "steam_users.db")

# Minimal price formatter used by some cogs
def get_price_string(price_overview: Optional[Dict] = None) -> str:
    if not price_overview:
        return "Price unknown"
    return price_overview.get("final_formatted") or price_overview.get("initial_formatted") or "Price unknown"

# Minimal embed creators to satisfy imports; these produce simple embeds and can be replaced
def create_comparison_embed(game1: Dict, game2: Dict) -> discord.Embed:
    name1 = (game1.get("data") or game1).get("name") if isinstance(game1, dict) else str(game1)
    name2 = (game2.get("data") or game2).get("name") if isinstance(game2, dict) else str(game2)
    e = discord.Embed(title=f"Comparison: {name1} vs {name2}", color=discord.Color.dark_gold())
    e.add_field(name=name1, value="Details unavailable in fallback", inline=True)
    e.add_field(name=name2, value="Details unavailable in fallback", inline=True)
    return e

def create_recommendation_embed(rec: Dict) -> discord.Embed:
    details = rec.get("details", {}) if isinstance(rec, dict) else {}
    name = details.get("name") or rec.get("name") or "Unknown"
    e = discord.Embed(title=f"Recommendation: {name}", description=", ".join(rec.get("match_reasons", [])) or "No reasons available", color=discord.Color.green())
    return e

# Lightweight View stubs so imports succeed during cog load.
class ComparisonView(discord.ui.View):
    def __init__(self, game1, game2, session=None):
        super().__init__(timeout=180)
        try:
            gid1 = game1.get("id") if isinstance(game1, dict) else str(game1)
            gid2 = game2.get("id") if isinstance(game2, dict) else str(game2)
            self.add_item(discord.ui.Button(label="🔗 Game 1", style=discord.ButtonStyle.link, url=f"https://store.steampowered.com/app/{gid1}"))
            self.add_item(discord.ui.Button(label="🔗 Game 2", style=discord.ButtonStyle.link, url=f"https://store.steampowered.com/app/{gid2}"))
        except Exception:
            pass
...
class RecommendationView(discord.ui.View):
    def __init__(self, recommendations, user):
        ...
        self._index = 0
        # Add a safe "View on Steam" link for the first recommendation
        if self.recommendations:
            first = self.recommendations[0]
            appid = first.get("app_id") or first.get("id")
            if appid:
                self.add_item(discord.ui.Button(label="🔗 View on Steam", style=discord.ButtonStyle.link, url=f"https://store.steampowered.com/app/{appid}"))

class EnhancedGameView(discord.ui.View):
    """Minimal fallback used when full EnhancedGameView isn't available."""
    ...
    async def create_main_embed(self) -> discord.Embed:
        """Return a basic embed representing the game (fallback)."""
        name = self.app_data.get("name", f"App {self.appid}")
        desc = self.app_data.get("short_description", "") if isinstance(self.app_data, dict) else ""
        e = discord.Embed(title=name, description=(desc[:4000] if desc else "No description available."), url=f"https://store.steampowered.com/app/{self.appid}", color=discord.Color.blurple())
        header = (self.app_data.get("header_image") or self.app_data.get("header")) if isinstance(self.app_data, dict) else None
        if header:
            try:
                e.set_image(url=header)
            except Exception:
                pass
        # expose screenshots/movies if present (expected by other code paths)
        self.screenshots = [s.get("path_thumbnail") if isinstance(s, dict) and s.get("path_thumbnail") else s for s in (self.app_data.get("screenshots") or [])] if isinstance(self.app_data, dict) else []
        self.movies = self.app_data.get("movies", []) if isinstance(self.app_data, dict) else []
        return e

class ScreenshotView(discord.ui.View):
    def __init__(self, screenshots, title="Screenshots"):
        super().__init__(timeout=180)
        self.screenshots = screenshots or []
        self.title = title

    def create_screenshot_embed(self, idx: int) -> discord.Embed:
        idx = max(0, min(idx, len(self.screenshots) - 1)) if self.screenshots else 0
        url = self.screenshots[idx] if self.screenshots else None
        e = discord.Embed(title=f"📷 {self.title} — {idx+1}/{max(1, len(self.screenshots))}", color=discord.Color.dark_gray())
        if url:
            try:
                e.set_image(url=url)
            except Exception:
