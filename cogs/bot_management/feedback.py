...
class Feedback(commands.Cog):
    ...
    # ---------------- Admin utilities ----------------
    @app_commands.command(name="feedback_mappings_list", description="List active feedback mappings (admin only)")
    async def mappings_list(self, interaction: discord.Interaction):
        # Restrict to bot owner or mod role if configured
        is_owner = await self.bot.is_owner(interaction.user)
        mod_role_id = getattr(config, 'MOD_ROLE_ID', None)
        is_mod = False
        if interaction.guild and mod_role_id:
        ...
        try:
            rows = await database.execute_db_operation(
                "list feedback mappings",
                "SELECT message_id, user_id, thread_id, created_at FROM feedback_messages ORDER BY created_at DESC",
                fetch_type='all'
            ) or []

            for message_id, user_id, thread_id, created_at in rows:
                entries.append(f"msg={message_id} user={user_id} thread={thread_id} created={created_at}")

        except Exception:
            logger.exception("Failed to fetch feedback mappings for admin list")
            await interaction.response.send_message("❌ Failed to fetch mappings.", ephemeral=True)
            return

        if not entries:
            await interaction.response.send_message("No feedback mappings found.", ephemeral=True)
            return

        # Send up to 2000 chars in response, otherwise attach as file
        out = "\n".join(entries)
        if len(out) > 1900:
            await interaction.response.send_message("Mappings are large; sending as file...", ephemeral=True)
            await interaction.followup.send(file=discord.File(io.StringIO(out), filename="feedback_mappings.txt"))
        else:
            # Send as a code block; include explicit newlines to avoid any
            # accidental unterminated string issues.
            await interaction.response.send_message(f"```\n{out}\n```", ephemeral=True)

    @app_commands.command(name="feedback_mappings_rebuild", description="Scan feedback channel and attempt to rebuild mappings (admin only)")
    async def mappings_rebuild(self, interaction: discord.Interaction):
        is_owner = await self.bot.is_owner(interaction.user)
        mod_role_id = getattr(config, 'MOD_ROLE_ID', None)
        is_mod = False
        if interaction.guild and mod_role_id:
        ...
        await interaction.response.send_message("🔎 Rebuilding feedback mappings — scanning channel...", ephemeral=True)

        channel = self.bot.get_channel(CHANNEL_ID)
        if not channel:
            await interaction.followup.send("❌ Feedback channel not found.")
            return
        ...
        try:
            async for msg in channel.history(limit=2000):
                # Try to extract submitter from embed author/footer if present
                uid = None
                if msg.embeds:
                    e = msg.embeds[0]
                    # If footer text contains 'Submitted by' and an ID, attempt parse
                    footer = getattr(e, 'footer', None)
                    if footer and footer.text:
                        m = None
                        import re
                        m = re.search(r"ID: (\d{17,20})", footer.text)
                        if m:
                            try:
                                uid = int(m.group(1))
                            except Exception:
                                uid = None
                # Fallback: try message.content for mention like <@123>
                if not uid and msg.content:
                    import re
                    m = re.search(r"<@!?(\d{17,20})>", msg.content)
                    if m:
                        try:
                            uid = int(m.group(1))
                        except Exception:
                ...
                if uid:
                    ...
                    try:
                        await database.execute_db_operation(
                            "upsert feedback mapping",
                            "INSERT OR REPLACE INTO feedback_messages (message_id, user_id, thread_id) VALUES (?, ?, ?)",
                            (msg.id, uid, None)
                        )
                        updated += 1
                    except Exception:
                        failed += 1
                        logger.exception(f"Failed to upsert mapping for message {msg.id}")

            await interaction.followup.send(f"✅ Rebuild complete. Found={found} updated={updated} failed={failed}")
        except Exception:
            logger.exception("Failed to rebuild feedback mappings")
            await interaction.followup.send("❌ Rebuild failed; check logs.")
