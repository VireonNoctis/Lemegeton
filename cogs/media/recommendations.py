...
# Recommendation Context System (TimeContext only)
class TimeContext(Enum):
    QUICK_READ = "quick_read"  # Short series or one-shots
    MODERATE = "moderate"     # Medium length series
    LONG_COMMITMENT = "long_commitment"  # Long-running series
    BINGE_WORTHY = "binge_worthy"  # Highly engaging, hard to put down

# Time context to chapter/episode ranges
TIME_CONTEXT_RANGES = {
    TimeContext.QUICK_READ: {"max_chapters": 50, "max_episodes": 12},
    TimeContext.MODERATE: {"max_chapters": 200, "max_episodes": 50},
    TimeContext.LONG_COMMITMENT: {"max_chapters": None, "max_episodes": None},
    TimeContext.BINGE_WORTHY: {"min_score": 85, "popular_threshold": 1000}
}
...
class Recommendations(commands.Cog):
    ...
    # ===== SMART RECOMMENDATION CONTEXT SYSTEM =====
    

    async def get_time_aware_recommendations(self, recommendations: List[Dict], time_context: TimeContext) -> List[Dict]:
        """Filter recommendations based on time context preferences"""
        if not recommendations:
            return []
        
        context_config = TIME_CONTEXT_RANGES.get(time_context, {})
        filtered_recs = []
        
        for rec in recommendations:
            media_details = rec.get('details') or rec.get('media_details', {})
            if not media_details:
                continue
            
            chapters = media_details.get('chapters')
            episodes = media_details.get('episodes')
            avg_score = media_details.get('averageScore', 0)
            popularity = media_details.get('popularity', 0)
            
            # Apply time context filtering
            if time_context == TimeContext.QUICK_READ:
                if chapters and chapters > context_config.get('max_chapters', 50):
                    continue
                if episodes and episodes > context_config.get('max_episodes', 12):
                    continue
            
            elif time_context == TimeContext.MODERATE:
                if chapters and chapters > context_config.get('max_chapters', 200):
                    continue
                if episodes and episodes > context_config.get('max_episodes', 50):
                    continue
            
            elif time_context == TimeContext.BINGE_WORTHY:
                min_score = context_config.get('min_score', 85)
                pop_threshold = context_config.get('popular_threshold', 1000)
                if avg_score < min_score or popularity < pop_threshold:
                    continue
    ...
    async def get_sequel_recommendations(self, user_id: int, guild_id: int) -> List[Dict]:
        """Get sequel/prequel recommendations based on user's completed series"""
        try:
            from database import get_user_guild_aware
            user_row = await get_user_guild_aware(user_id, guild_id)
            if not user_row:
                return []
            
            anilist_username = user_row[4]
            if not anilist_username:
                return []
            
            # Fetch user's completed entries
            entries = await self._fetch_user_manga_with_recs(anilist_username, per_page=100)
            completed_entries = [e for e in entries if e.get('status') == 'COMPLETED' and e.get('score', 0) >= 70]
            ...
            for entry in completed_entries:
                ...
                # Query for related media (sequels, prequels, spin-offs)
                relations_query = """
                query ($id: Int) {
                    Media(id: $id) {
                        relations {
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(ANILIST_API_URL, json={
                            "query": relations_query, 
                            "variables": {"id": media_id}
                        }) as resp:
                            if resp.status == 200:
                                data = await resp.json()
                                relations = data.get('data', {}).get('Media', {}).get('relations', {}).get('edges', [])
                                
                                for relation in relations:
                                    rel_type = relation.get('relationType')
                                    if rel_type in ['SEQUEL', 'PREQUEL', 'SIDE_STORY', 'SPIN_OFF']:
                                        node = relation.get('node', {})
                                        if node.get('type') == 'MANGA' and node.get('status') in ['RELEASING', 'FINISHED']:
                                            sequel_candidates.append({
                                                'media_id': node.get('id'),
                                                'title': node.get('title', {}).get('romaji', 'Unknown'),
                                                'relation_type': rel_type,
                                                'original_title': media.get('title', {}).get('romaji', 'Unknown'),
                                                'media_details': node
                                            })
                except Exception as e:
                    logger.debug(f"Error fetching relations for media {media_id}: {e}")
                    continue
            
            # Remove duplicates and sort by relevance
            seen_ids = set()
            unique_sequels = []
            for candidate in sequel_candidates:
                media_id = candidate['media_id']
                if media_id not in seen_ids:
                    seen_ids.add(media_id)
                    unique_sequels.append(candidate)
            
            return unique_sequels[:10]
            
        except Exception as e:
            logger.error(f"Error getting sequel recommendations: {e}")
            return []
    ...
    async def recommendations(self, interaction: discord.Interaction, member: Optional[discord.Member] = None, rec_type: app_commands.Choice[str] = None):
        ...
        loading_message = (
            ...
            "📊 Analyzing your highly-rated manga (7.0+ scores)\n"
            "🌐 Fetching global recommendation data from AniList\n"
            "🎯 Building personalized suggestions\n\n"
            "⏰ **Please wait 10-30 seconds** - Quality analysis takes time!"
        )
        
        # Send ephemeral message with wait time estimate
        await interaction.response.send_message(loading_message, ephemeral=True)
        ...
        if entries is None:
            logger.error(f"Failed to fetch entries for {anilist_username} - likely user not found")
        ...
        # For personal recommendations, use the user's own entries
        for e in entries:
        ...
        logger.info(f"User '{anilist_username}' has {len(user_media_ids)} titles on their list for exclusion.")
        ...
        # For personal recommendations, use original logic
        scored_entries.sort(key=lambda x: _normalize_score(x.get("score")), reverse=True)
        top_n = 100
        top_entries = scored_entries[:top_n]
        logger.info(f"Personal recommendations: Selected top {len(top_entries)} rated entries out of {len(scored_entries)} scored entries.")
        ...
        min_score_threshold = 70  # Only from manga rated 7.0+ (70/100)
        ...
        if not tally:
            await interaction.followup.send("No recommendations found from highly-rated manga (7.0+). Try rating more manga!")
            return
    ...
    # Helper method for score normalization
