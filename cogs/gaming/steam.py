    ...
    try:
        ...
        try:
            ...
            from helpers.steam_helper import (
                logger, safe_json, fetch_text, chunk_list, random_color, human_hours,
                safe_text, make_friend_grid_image, STEAM_API_KEY, DB_PATH, PIL_AVAILABLE,
                ComparisonView, RecommendationView, ScreenshotView,
                create_comparison_embed, get_price_string, create_recommendation_embed
            )

            # Provide a minimal EnhancedGameView fallback if it's not exported by the helper
            if 'EnhancedGameView' not in globals():
                class EnhancedGameView(discord.ui.View):
                    """Minimal fallback view used when the full EnhancedGameView isn't available.

                    This provides a simple 'View on Steam' link so code paths expecting
                    an EnhancedGameView can still function without the richer UI.
                    """
                    def __init__(self, app_data, appid, session, user):
                        super().__init__(timeout=180)
                        url = f"https://store.steampowered.com/app/{appid}"
                        self.add_item(discord.ui.Button(label="🔗 View on Steam", style=discord.ButtonStyle.link, url=url))
...
class Steam(commands.Cog):
    ...
    # ---------------- Profile ----------------
    @steam_group.command(name="profile", description="Show a Steam profile (vanity or SteamID)")
    async def profile(self, interaction: discord.Interaction, user: Optional[str] = None):
...
# -------------------- Recommendations command --------------------
    @steam_group.command(name="recommendations", description="Get personalized game recommendations")
    ...
    @steam_group.command(name="compare", description="Compare two games side by side")
    @app_commands.describe(game1="First game", game2="Second game")
    ...
    @app_commands.describe(max_price="Maximum price (USD)", genre="Filter by genre", discount_min="Minimum discount percent")
    async def find_deals(self, interaction: discord.Interaction, max_price: Optional[float] = None, genre: Optional[str] = None, discount_min: Optional[int] = None):
    ...
    @app_commands.describe(category="Category (new_releases/top_sellers/popular/upcoming)")
    async def trending_games(self, interaction: discord.Interaction, category: Optional[str] = "popular"):
